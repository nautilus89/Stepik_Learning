...
def draw_pane(id):
    ...
    if (id == 1):
        ...
        pane_number = 3;
        markup = types.ReplyKeyboardMarkup(row_width=2)
        itembtn1 = types.KeyboardButton('thx')
        itembtn2 = types.KeyboardButton('change location')
        markup.add(itembtn1, itembtn2)
        bot.send_message(chat_id, "wanna change your location?", reply_markup=markup)
...
############################################################################ PARISNG ##################################################################################
def parsing():
    url = 'https://ru.investing.com/equities/tesla-motors-chart'
    r = requests.get(url)
    print(r.text)
